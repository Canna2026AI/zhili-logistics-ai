...
- 所有产品端和官网按钮均使用纯色：默认 `teal/700`、悬停 `teal/600`、按压 `teal/800`；不定义品牌渐变令牌。
...
组件级状态证据还必须覆盖：加载骨架、空数据、请求失败、无权限、`If-Match` 数据过期、批量部分成功、危险确认、长文本、字段脱敏和百万级列表虚拟滚动。PDA 组件额外覆盖离线、队列满、照片补传、登录失效、冲突与重启恢复。
...
- 每个页面只有一个与当前状态匹配的实心主命令。跨状态组合命令放入二级菜单，先展示前置检查、将创建的记录、费用和不可撤销后果。
- AppShell、Drawer、命令层级、终端画布和跨域状态映射以 `app-shell-spec.md` 为唯一交互源真相。
- 流程异常态和补救动作以 `interaction-state-matrix.md` 为唯一状态源真相。

## 5. 图标、圆角与终端差异

- 业务 UI 图标统一为 24px viewport、1.75–2px 圆角线性 SVG；状态必须同时有文字。
- 官网可以使用独立的物流 AI 插画，但营销插画不得进入业务组件库。
- 业务桌面控件为 4–6px，信息容器上限 8px；PDA 触控卡可用 8–12px；胶囊只允许状态标签和浮动快捷操作。
- 客户门户可降低密度，但任务优先级高于装饰性 KPI；没有配置价值的空模块不占据首屏。
