...
实现按红—绿—重构推进：第四轮先新增并确认 7 个失败测试，覆盖 typed quote/accept、订单报价关联、410、展示期间跨期、运行实体合并和 production preview 404，再补实现并通过回归。

- 单元/集成测试：客户门户 22/22、平台端 11/11、官网 11/11（含 5 个真实 production preview 探针），总计 44/44。
