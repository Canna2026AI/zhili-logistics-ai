...
test('F1A 下单报价保留标准数据、查价解释与唯一主命令', async ({ page }) => {
  ...
  await page.getByLabel('高 (cm)').fill('200');
  ...
  await expect(page.getByText('UPS 计费重为 1333.33 kg')).toBeVisible();
  await page.getByLabel('目的地邮编').fill('91761');
  await expect(page.getByText(/RATE-UPS-CN-US-2026\.05-v5/)).not.toBeVisible();
  await expect(page.getByRole('button', { name: '查看解释' })).toBeDisabled();
...
test('F1A 权限模拟不把拒绝伪装成空数据', async ({ page }) => {
  ...
  const drawer = page.getByRole('dialog', { name: '运单详情' });
  await expect(drawer).toContainText('139 **** 8800');
  await expect(drawer).not.toContainText('139 2654 8800');
  await expect(drawer.getByRole('button', { name: '问题件登记' })).toBeDisabled();
  await page.getByRole('button', { name: '关闭' }).click();
